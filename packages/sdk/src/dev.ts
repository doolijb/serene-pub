...
export function reloadPlan(prev: DevOverlay, next: DevOverlay, inFlight: ReadonlyArray<{ runId: string; specId: string; typeIds: string[] }> = []): ReloadPlan {
	...
	for (const [id, t] of nextTypes)
		if (!prevTypes.has(id)) changes.push({ kind: 'type-added', id, hot: true, note: 'nothing can be using it yet' })
	...
	for (const id of prevTypes.keys())
		if (!nextTypes.has(id)) changes.push({ kind: 'type-removed', id, hot: false, note: 'a run may be mid-node' })
	...
	for (const [id, d] of nextDocs)
		if (!prevDocs.has(id)) changes.push({ kind: 'pipeline-added', id, hot: true })
	...
	for (const id of prevDocs.keys()) if (!nextDocs.has(id)) changes.push({ kind: 'pipeline-removed', id, hot: false })
	...
	const touching = (c: Change) =>
		inFlight.filter((r) => r.specId === c.id || r.typeIds.some((t) => c.id.startsWith(t))).map((r) => r.runId)
