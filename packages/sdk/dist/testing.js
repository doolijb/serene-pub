...
export function diffGolden(before, after) {
    ...
    const walk = (a, b, path) => {
        ...
        if (!both)
            return void out.push({ path, before: a, after: b });
...
export function checkGolden(name, r, stored) {
    ...
    if (!stored)
        return { golden, recorded: true };
    const diff = diffGolden(stored, golden);
    if (diff.length)
        throw new GoldenMismatch(name, diff);
...
const isResult = (v) => !!v && typeof v === 'object' && ['ok', 'err', 'cancelled', 'halt'].includes(v.kind);
/**
...
export const BINDING_PROBES = [
    {
        ...
        consequence: 'The executor cannot tell success from a halt, so a correct "not applicable" is recorded as an error and the run inspector stops being trustworthy.',
    ...
    },
    {
        ...
        consequence: 'Purity is what makes replay exact (F11). A Task that reads the clock or the network produces a run nobody can reproduce, and the receipt becomes a story rather than a record.',
    ...
    },
    {
        ...
        consequence: 'A dice roll nobody can replay is a dice roll nobody can dispute — and the seeded version is the whole reason SP can explain a probability roll where the incumbent cannot.',
        async check(hook, d, ctx) {
            ...
            must(JSON.stringify(a) === JSON.stringify(b), 'two invocations with the same seeded RNG produced different output — something is reaching for Math.random');
        },
    },
    {
        id: 'B4',
        title: 'it settles rather than hanging',
        consequence: '`runtime: node` abandons rather than kills (13 §7h), so a hook that never settles keeps burning CPU after the executor has moved on and eventually marks your plugin unhealthy.',
        async check(hook, _d, ctx) {
    ...
    },
    {
        ...
        consequence: 'The signal is the only cooperative way an in-process hook can be stopped. Ignoring it means an admin killing a run watches it keep going.',
        async check(hook, _d, ctx) {
...
function bounded(p) {
    ...
    return Promise.race([
        p,
        new Promise((_, rej) => {
            timer = setTimeout(() => rej(new Error(`probe did not settle within ${PROBE_TIMEOUT_MS}ms — the hook never returned`)), PROBE_TIMEOUT_MS);
        }),
...
export async function probeBinding(hook, descriptor, ctx) {
    ...
    for (const p of BINDING_PROBES) {
        try {
            ...
            out.push({ id: p.id, title: p.title, pass: false, error: e.message, consequence: p.consequence });
        }
    }
    return out;
}
...
export const probeCtxFor = (kind, sampleInput = {}) => ({
    ...
    makeCtx: (over = {}) => {
        ...
        if (kind === 'consumer') {
            base.commit = async (p) => ({ id: 'row:probe', ...p });
            base.emit = () => { };
        }
        return { ...base, ...over };
    },
...
export async function assertEquivalent(doc, opts) {
    ...
    if (norm(par) !== norm(seq)) {
        throw new Error('forced-sequential execution produced a different result (F26). Something in this spec ' +
