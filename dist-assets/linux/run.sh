...
#
# The one thing this adds over calling app/run.sh directly: on failure, if
# someone is sitting at an interactive terminal (as when double-clicking this
# file from a file manager), it waits for a keypress so the window doesn't
# close before they can read the error. A successful run never pauses, and
# SERENE_PUB_NO_PAUSE=1 (or "true") skips the pause unconditionally.
