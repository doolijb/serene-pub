<script lang="ts">
	// Picker shown from EditChatForm's "Removed" section: lets the caller
	...
	// list UI — the server (chats:reassignRemovedParticipant) does the real
...
{:else}
	<PersonaSelectModal
		...
		description={`Give a persona ${removedName}'s message history in this chat.`}
	/>
{/if}
