...
<Dialog {open} {onOpenChange}>
	<Portal>
		...
		>
			<Dialog.Content
				class="card bg-surface-100-900 p-6 shadow-xl {size === 'lg'
					? 'w-[min(95vw,720px)]'
					: 'w-[min(95vw,600px)]'} max-h-[90vh] overflow-y-auto"
			>
				{#if step === "confirm"}
					<header class="mb-4 flex items-center justify-between">
						<h2 class="h3">{title}</h2>
						{#if badge}<span
								class="badge preset-tonal-secondary text-xs"
							>
								{badge}
							</span>{/if}
					</header>
				...
				{:else if step === "error"}
					...
					<div
						class="border-error-500/30 bg-error-500/10 flex items-start gap-3 rounded-lg border p-3 text-sm"
					>
						<Icons.AlertCircle
							size={16}
							class="text-error-500 mt-0.5 shrink-0"
						/>
						<span>
					...
					<footer class="mt-6 flex flex-wrap gap-3">
						...
						{/if}
						<button
							class="btn preset-filled-primary-500 {onStartOver
								? ''
								: 'ml-auto'}"
							onclick={onRetry}
						>
