			...
			>
				...
				{#if !selectedParent}
					<p class="text-surface-700-300 text-sm">
						Select the node that <strong>{node.name}</strong>
						is an alias of. The selected node will become the primary
						entity; "{node.name}" will be hidden and treated as an
						alternate name.
					</p>
					...
					<div class="space-y-4">
						...
						>
							...
							<p class="text-surface-700-300 text-xs">
								"{node.name}" will be hidden from the graph and
								treated as an alternate name for "{selectedParent.name}".
								Its relationships will fall back to "{selectedParent.name}"
							...
							{#if node.lorebookBindingId && !selectedParent.lorebookBindingId}
								<p class="text-warning-500 text-xs">
									The character binding from "{node.name}"
									will transfer to "{selectedParent.name}".
								</p>
							{/if}
						</div>
