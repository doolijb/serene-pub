<script lang="ts">
	...
	let sceneActivityBySceneId = $derived.by(() => {
		const map = new Map<number, (typeof sceneSummarizesCtx.activities)[number]>()
	...
	let compileActivityByEntryId = $derived.by(() => {
		const map = new Map<number, (typeof compileEntriesCtx.activities)[number]>()
	...
	let processModalPendingResult = $state<SceneSummarizeState["pendingResult"] | null>(null)
	...
	$effect(() => {
		...
		const activity = sceneSummarizesCtx.activities.find(
			(a) => a.sceneId === id && (a.status === "review" || a.status === "running")
		)
		if (!activity) return
	...
	function openCompileModal(
		...
		opts?: { activityId?: string | null; pendingResult?: { content: string } | null; initialStep?: "review" | "running" }
	) {
	...
	$effect(() => {
		...
		if (!reviewId) return
		const activity = compileEntriesCtx.activities.find((a) => a.historyEntryId === reviewId)
		if (!activity) return
	...
	onMount(() => {
		...
		socket.on("scenes:process:error", (msg: Sockets.Scenes.Process.ErrorResponse) => {
	...
	<div class="flex flex-col gap-3">
		...
		{:else}
			{#each filteredEntries as entry}
				...
				{@const entryCompileActivity = compileActivityByEntryId.get(entry.id)}
					...
					<div class="min-w-0 flex-1">
						...
						<div class="mt-1.5 flex flex-wrap items-center gap-1">
							...
							{#if entryCompileActivity?.status === "running"}
								<span class="preset-filled-tertiary-500 rounded px-1.5 py-0.5 text-xs" title="Compiling…">
									<Icons.Loader size={11} class="inline animate-spin" /> Compiling…
								</span>
							{:else if entryCompileActivity?.status === "review"}
								<span class="preset-filled-warning-500 rounded px-1.5 py-0.5 text-xs" title="Review pending">
	...
	<div class="flex flex-col gap-4">
		...
		<div class="flex items-center gap-2">
			...
			<h3 class="flex-1 text-sm font-semibold">
				Year {focusedEntry.year}{focusedEntry.month ? `, Mo. ${focusedEntry.month}` : ""}{focusedEntry.day ? `, Day ${focusedEntry.day}` : ""}
			</h3>
		</div>
		...
		{#if focusedEntryTab === "content"}
			<div class="flex flex-col gap-3 text-sm">
				<button class="btn btn-sm preset-filled-primary-500 self-start" onclick={() => editEntry(focusedEntry!)}>
		...
		{:else}
			...
			{:else if focusedSceneId !== null}
				{@const scene = viewScenes.find((s) => s.id === focusedSceneId)}
				{#if scene}
					{@const sceneActivity = sceneActivityBySceneId.get(scene.id)}
					{@const isProcessing = sceneActivity?.status === "running"}
					{@const pendingActivity = sceneActivity?.status === "review" ? sceneActivity : undefined}
					...
					<div class="flex flex-col gap-4">
						<div class="flex items-center gap-2">
							...
							{#if scene.graphed}
								<Icons.GitGraph size={13} class="text-success-500 shrink-0" />
							{/if}
						</div>
						<div class="flex flex-wrap gap-2">
							{#if pendingActivity}
								<button
									...
									onclick={() => openProcessModal(pendingActivity)}
								><Icons.Eye size={13} /> Review Pending</button>
							{:else}
								<button
			...
			{:else}
				...
				<div class="flex flex-col gap-2">
					{#if ungraphedSummarized.length > 0 && onNavigateToGraph}
						<button class="btn btn-sm preset-tonal-secondary w-full" onclick={onNavigateToGraph}>
					...
					{:else}
						{#each viewScenes as scene, sceneIdx}
							...
							<div
								role="button"
								tabindex="0"
								class="bg-surface-200-800 hover:bg-surface-300-700 flex flex-col gap-1.5 rounded-md p-2 text-sm cursor-pointer transition-colors"
								onclick={() => (focusedSceneId = scene.id)}
							>
								<div class="flex items-center gap-1.5 min-w-0">
									<span class="flex-1 truncate font-medium text-sm">
										<span class="text-surface-400 font-normal mr-1">{sceneIdx + 1}.</span>{scene.name ?? "Unnamed Scene"}
									</span>
									{#if scene.graphed}
										<Icons.GitGraph size={12} class="text-success-500 shrink-0" />
									{/if}
									{#if isProcessing}
										<Icons.Loader size={13} class="animate-spin text-primary-500 shrink-0" />
									{:else if pendingActivity}
										<button
											class="btn btn-sm p-1 preset-filled-warning-500 shrink-0"
											onclick={(e) => { e.stopPropagation(); openProcessModal(pendingActivity) }}
								...
								{#if scene.summary}
									<p class="text-surface-600-400 line-clamp-2 whitespace-pre-wrap text-xs leading-relaxed">{scene.summary}</p>
								{/if}
							</div>
						{/each}
						<button class="btn btn-sm preset-tonal-secondary w-full" onclick={() => openCompileModal(focusedEntry!)}>
				...
				<div class="flex flex-col gap-2">
					...
					{:else}
						{#each editScenes as scene, sceneIdx}
							{@const sceneActivity = sceneActivityBySceneId.get(scene.id)}
							{@const isProcessing = sceneActivity?.status === "running"}
							{@const pendingActivityEdit = sceneActivity?.status === "review" ? sceneActivity : undefined}
							{@const hasMessages = (scene.selectedMessageIds?.length ?? 0) > 0}
							{@const hasSummary = !!scene.summary}
							<div class="bg-surface-200-800 flex flex-col gap-2 rounded-md p-2 text-sm">
								<div class="flex items-center gap-1.5 min-w-0">
									...
									{#if scene.graphed}
										<Icons.GitGraph size={12} class="text-success-500 shrink-0" aria-label="Added to graph" />
									{/if}
									{#if isProcessing}
										<Icons.Loader size={13} class="animate-spin text-primary-500 shrink-0" />
...
{#if showProcessModal && processModalSceneId !== null}
	<ProcessSceneModal
		...
		onApplied={(_sceneId) => { showProcessModal = false }}
		onDiscarded={(_activityId) => { showProcessModal = false }}
	/>
{/if}
