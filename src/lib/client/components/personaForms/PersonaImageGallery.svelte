<script lang="ts">
	...
	let { personaId, currentAvatar = $bindable(), onAvatarChange }: Props = $props()
	...
	function selectAvatar(imgPath: string) {
		socket.emit("personas:setAvatar", { personaId, path: imgPath })
	}
	...
	onMount(() => {
		socket.on("personas:listGallery", (msg) => {
			isLoading = false
			images = msg.images
		})

		socket.on("personas:uploadGalleryImage", (msg) => {
			isUploading = false
			if (msg.success) {
		...
		socket.on("personas:uploadGalleryImage:error" as any, () => {
			isUploading = false
			toaster.error({ title: "Upload failed" })
		})

		socket.on("personas:deleteGalleryImage", () => {
			toaster.success({ title: "Image deleted" })
		})

		socket.on("personas:setAvatar", (msg) => {
			toaster.success({ title: "Avatar updated" })
			if (msg.persona?.avatar) {
				currentAvatar = msg.persona.avatar
				onAvatarChange?.(msg.persona.avatar)
			}
		})

		socket.emit("personas:listGallery", { personaId })
	})
