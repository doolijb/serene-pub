<script lang="ts">
		...
		const state = EditorState.create({
			...
			extensions: [
				...
				EditorView.theme({
					...
					".cm-scroller": { overflow: "auto", fontFamily: "'Fira Mono', 'Cascadia Code', 'JetBrains Mono', monospace" },
	...
	function handleFileImport(e: Event) {
		...
		reader.onload = (ev) => {
			...
			let stripped = content.replace(/^\s*\[data-theme=[^\]]*\]\s*\{([\s\S]*)\}\s*$/, (_, inner) => inner.trim())
			if (stripped === content) stripped = content.replace(/^\s*\{([\s\S]*)\}\s*$/, (_, inner) => inner.trim())
			editorView?.dispatch({
				changes: { from: 0, to: editorView.state.doc.length, insert: stripped }
			})
			if (!labelField) {
				labelField = file.name.replace(/\.(css|json)$/i, "").replace(/[_-]/g, " ").replace(/\b\w/g, c => c.toUpperCase())
			}
		}
	...
	onMount(() => {
		...
		socket.on("customThemes:getCss", (msg: Sockets.CustomThemes.GetCss.Response) => {
			...
			isLoadingCss = false
			editorView?.dispatch({
				changes: { from: 0, to: editorView.state.doc.length, insert: msg.css }
			})
		})
...
>
	<!-- Editor toolbar -->
	<div class="flex items-center gap-2 border-b px-4 py-2" style="border-color: #2a2a3a; background: #1a1a2e;">
		<div class="flex flex-1 items-center gap-3">
			<!-- Dot accent -->
			<span class="h-2.5 w-2.5 rounded-full" style="background: linear-gradient(135deg, #7c6af7, #a855f7);"></span>
			<span class="text-xs font-semibold tracking-wider uppercase" style="color: #8b8ba7;">Custom Theme</span>
		</div>
		<div class="flex items-center gap-1">
			<!-- Import file -->
			<label class="btn btn-sm cursor-pointer text-xs" style="color: #8b8ba7; background: transparent;" title="Import CSS/JSON file">
				<Icons.Upload size={13} />
				<span>Import</span>
				<input type="file" accept=".css,.json" class="hidden" onchange={handleFileImport} />
			</label>
			<!-- Fullscreen toggle -->
			<button
	...
	{#if isLoadingCss}
		<div class="flex flex-1 items-center justify-center" style="background: #13131f;">
			<Icons.Loader2 size={24} class="animate-spin" style="color: #7c6af7;" />
		</div>
	{/if}
	...
	<!-- Status bar -->
	<div class="flex items-center justify-between border-t px-4 py-1.5" style="border-color: #2a2a3a; background: #1a1a2e;">
		...
		<div class="flex items-center gap-2">
			...
			{#if isAdmin && theme && isAccountsEnabled}
				<button
					class="btn btn-sm text-xs"
					style="color: {theme.isInstanceTheme ? '#a855f7' : '#4a4a6a'}; background: transparent; border: 1px solid {theme.isInstanceTheme ? '#7c6af7' : '#2a2a3a'};"
					onclick={() => toggleInstanceTheme(!theme!.isInstanceTheme)}
					title={theme.isInstanceTheme ? "Disable for all users" : "Enable for all users"}
				>
					<Icons.Globe size={11} />
					{theme.isInstanceTheme ? "Instance theme" : "Make instance theme"}
				</button>
			{/if}
			...
			{#if theme?.id}
				{#if confirmDelete}
					<button
						...
						disabled={isDeleting}
					>
						{#if isDeleting}<Icons.Loader2 size={11} class="animate-spin" />{:else}<Icons.Trash2 size={11} />{/if}
						Confirm delete
					</button>
					<button
						class="btn btn-sm text-xs"
