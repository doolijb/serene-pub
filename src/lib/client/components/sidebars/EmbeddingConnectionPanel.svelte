<script lang="ts">
	...
	$effect(() => {
		...
		const handleSetApiConfig = (
			message: Sockets.Vectorization.SetApiConfig.Response
		) => {
			testingApi = false
			if (message.success) {
				mode = "api"
				...
				apiTestError =
					message.error ?? "Failed to validate the embeddings API"
			}
		}
		...
		const handleGetQueue = (
			message: Sockets.Vectorization.GetQueue.Response
		) => {
			vectorizationCtx.priorityQueue = message.queue
			vectorizationCtx.history = message.history ?? []
		}
		const handleRemoveFromQueue = (
			message: Sockets.Vectorization.RemoveFromQueue.Response
		) => {
			vectorizationCtx.priorityQueue = message.queue
			removingGroup = null
		}
		const handleMoveQueueGroup = (
			message: Sockets.Vectorization.MoveQueueGroup.Response
		) => {
			vectorizationCtx.priorityQueue = message.queue
		}
		const handleVectorizationConfigUpdate = () => {
			savingTtl = false
		}
		...
		return () => {
			...
			socket.off(
				...
				handleVectorizationConfigUpdate
			)
		}
	})
	...
	function confirmChangeModel() {
		...
		socket?.emit("vectorization:setModel", {
			modelName: selectedModelForChange
		})
	}
	...
	const activeModelDef = $derived(
		availableModels.find((m) => m.id === activeModelName)
	)
	...
	$effect(() => {
		if (!socket) return
		const handleVecConfig = (
			res: Sockets.VectorizationConfig.Get.Response
		) => {
	...
	function saveTtl() {
		...
		savingTtl = true
		socket?.emit("vectorizationConfig:update", {
			embeddingModelTtlMinutes: val
		})
	}
		...
		<div class="flex flex-col gap-4 overflow-y-auto pb-4">
							...
							>
								...
								<div class="min-w-0 flex-1">
									...
									>
										...
										{#if model.tier === "fast"}
											<span
												class="rounded-full bg-sky-500/15 px-1.5 py-0.5 text-xs font-medium text-sky-600"
											>
												Fast
											</span>
										{:else if model.tier === "balanced"}
											<span
												class="rounded-full bg-violet-500/15 px-1.5 py-0.5 text-xs font-medium text-violet-600"
											>
												Balanced
											</span>
										{:else}
											<span
												class="rounded-full bg-amber-500/15 px-1.5 py-0.5 text-xs font-medium text-amber-600"
											>
												Best
											</span>
										{/if}
										...
										>
											{model.sizeLabel} · {model.dimensions}d
										</span>
									</div>
									<p
										class="text-surface-700-300 mt-0.5 text-xs"
									>
										{model.description}
									</p>
								</div>
			...
			{:else if settingsView === "setup-api"}
				...
				<p class="text-surface-700-300 text-xs">
					Any OpenAI-compatible /embeddings endpoint — OpenAI itself,
					Ollama, LM Studio, llama.cpp server, etc.
				</p>

				<label class="block">
					<span class="text-surface-700-300 text-xs">Base URL</span>
					<input
						type="text"
				...
				<label class="block">
					<span class="text-surface-700-300 text-xs">API Key</span>
					<input
						type="password"
						class="input w-full text-sm"
				...
				<label class="block">
					<span class="text-surface-700-300 text-xs">Model</span>
					<input
						type="text"
						class="input w-full text-sm"
								...
								{#each vectorizationCtx.priorityQueue as group, i (group.groupId)}
									...
									>
										...
										<div class="min-w-0 flex-1">
											...
											<p
												class="text-surface-700-300 truncate"
											>
												{group.ownerDisplayName}
											</p>
											{#if summary}
												<p
													class="text-surface-400 mt-0.5"
												>
													{summary}
												</p>
											{/if}
			...
			<Tabs.Content value="settings">
				<div class="flex flex-col gap-4 overflow-y-auto pb-4">
						...
						>
							...
							<div class="min-w-0 flex-1 space-y-2">
								{#if mode === "api"}
									...
									<p class="text-surface-700-300 text-xs">
										{modelLoadError ??
											"RAG retrieval is skipped (falls back to keyword search) until the embeddings API is reachable again."}
									</p>
								{:else if !modelCached}
									<p class="text-sm font-medium">
										Model files missing
									...
									<p class="text-surface-700-300 text-xs">
										The embedding model is not in the local
										cache. Re-download it to resume
										embeddings.
									</p>
								...
								{:else}
									...
									<p class="text-surface-700-300 text-xs">
										The server restarted and the model needs
										to be reloaded before embeddings can
										run.
									</p>
								{/if}
								{#if mode === "local"}
					...
					{#if mode === "local"}
						<!-- Current model card -->
						<div
							class="bg-surface-200-800 space-y-1 rounded-lg p-3"
						>
							...
							{#if activeModelDef}
								<div
									class="text-surface-700-300 flex flex-wrap items-center gap-1.5 text-xs"
								>
									...
									{/if}
									<span>
										{activeModelDef.dimensions}d · {activeModelDef.sizeLabel}
									</span>
								</div>
							...
							{#if !modelReady}
								<span
									class="bg-warning-500/20 text-warning-600 mt-1 inline-block rounded-full px-2 py-0.5 text-xs font-medium"
								>
									Not loaded
								</span>
							{:else}
								<span
									class="bg-success-500/20 text-success-600 mt-1 inline-block rounded-full px-2 py-0.5 text-xs font-medium"
								>
									Loaded
								</span>
							{/if}
						...
						{#if downloadProgress}
							<div class="space-y-1">
								...
								>
									<span
										class="text-surface-700-300 capitalize"
									>
										{downloadProgress.status}…
									</span>
									{#if downloadProgress.percent !== undefined}
						...
						>
							...
							<p class="text-surface-700-300 text-xs">
								Unload the embedding model after this many
								minutes of inactivity. Set to 0 to keep it
								loaded indefinitely.
							</p>
							<div class="flex items-center gap-2">
								<input
								...
								<button
									class="btn btn-sm preset-tonal-primary ml-auto text-xs"
									onclick={saveTtl}
									disabled={savingTtl}
								>
									{#if savingTtl}
		...
		>
			...
			>
				...
				<div class="text-warning-500 flex items-start gap-2 text-sm">
					...
					<p>
						Changing the model will stop the current embedding
						queue. All existing embeddings will need to be
						regenerated with the new model.
					</p>
				</div>
							...
							<div class="min-w-0 flex-1">
								<div class="flex flex-wrap items-center gap-2">
									...
									{#if model.id === activeModelName}
										<span
											class="bg-primary-500/20 text-primary-600 rounded-full px-1.5 py-0.5 text-xs"
										>
											current
										</span>
									{/if}
				...
				{#if downloadProgress}
					<div class="space-y-1">
						...
						{#if downloadProgress.percent !== undefined}
							<div
								class="bg-surface-300-600 h-1.5 w-full overflow-hidden rounded-full"
							>
								<div
									class="bg-primary-500 h-full transition-all duration-300"
									style="width: {downloadProgress.percent}%"
				...
				<footer class="flex justify-end gap-2">
					...
					<button
						...
						disabled={!selectedModelForChange ||
							selectedModelForChange === activeModelName ||
							isChangingModel}
					>
						{#if isChangingModel}
		...
		>
			...
			>
				...
				<footer class="flex justify-end gap-2">
					<button
						class="btn preset-filled-surface-400-600"
						onclick={() => (showDisableConfirmModal = false)}
					>
						Cancel
					</button>
					<button
