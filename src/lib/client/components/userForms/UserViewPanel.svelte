...
<div class="flex h-full flex-col gap-0 overflow-hidden">
	...
	<div class="flex flex-1 flex-col gap-4 overflow-y-auto">
		...
		<div class="flex items-center gap-3">
			<div
				class="preset-filled-surface-300-700 flex h-14 w-14 shrink-0 items-center justify-center rounded-full text-xl font-bold"
			>
		...
		<div class="flex flex-wrap gap-2">
			{#if user.isAdmin}
				<span
					class="preset-filled-primary-500 rounded px-2 py-0.5 text-xs font-medium"
				>
					Admin
				</span>
			{:else}
				<span
					class="preset-tonal-surface rounded px-2 py-0.5 text-xs font-medium"
				>
					User
				</span>
			{/if}
