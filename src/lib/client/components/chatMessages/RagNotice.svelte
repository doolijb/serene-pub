<script lang="ts">
	...
	let { chatId, totalMessages }: Props = $props()
	...
	$effect(() => {
		// Re-fetch whenever chatId or message count changes
		chatId
		totalMessages
		fetchStatus()
	})
	...
	function handleMoveToTop() {
		prioritizing = true
		socket.emit("vectorization:addToQueue", { chatId }, () => {
			fetchStatus()
			prioritizing = false
		})
	}
		...
		<div class="flex flex-wrap items-start gap-2">
			...
			<div class="flex shrink-0 flex-wrap gap-1">
				...
				<button
					...
					disabled={ignoring}
					title="Ignore RAG for this chat and hide this notice"
				>
					Ignore for this chat
				</button>
			</div>
		</div>
	</div>
