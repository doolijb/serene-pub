<script lang="ts">
	...
	let sortedPlatforms = $derived(
		Object.keys(grouped).sort((a, b) => (platformOrder[a] ?? 9) - (platformOrder[b] ?? 9))
	)
...
<div class="flex flex-col gap-4 p-3">
	...
	<!-- Download directory -->
	<div>
		<label class="text-surface-600-400 mb-1 block text-xs font-medium" for="binaryDestDir">
			Download directory
		</label>
		<input
			id="binaryDestDir"
	...
	{#if download && !download.isDone}
		...
		<div class="bg-surface-100-900 rounded-lg p-3">
			<div class="mb-2 flex items-center gap-2">
				...
				<span class="text-xs font-medium">Downloading {download.assetName}</span>
				<button
					class="btn btn-sm preset-tonal-error ml-auto text-xs"
					onclick={() => socket.emit("koboldcpp:cancelBinaryDownload", {})}
				>
					Cancel
				</button>
			</div>
	...
	{:else if download?.status === "success"}
		<div class="bg-success-100-900 text-success-700-300 flex items-center gap-2 rounded-lg p-3 text-xs">
			<Icons.CheckCircle size={14} />
			Download complete — KoboldCPP is starting…
		</div>
	{:else if download?.status === "error"}
		<div class="bg-error-100-900 text-error-700-300 flex items-center gap-2 rounded-lg p-3 text-xs">
