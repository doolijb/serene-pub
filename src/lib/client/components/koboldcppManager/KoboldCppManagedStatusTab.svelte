<script lang="ts">
	...
	let ttlSecs = $state(koboldCppSettingsCtx.settings?.koboldCppManagedModelTtlSecs ?? 300)
	let ttlDraft = $state(String(ttlSecs))
	let savingTtl = $state(false)

	let port = $state(koboldCppSettingsCtx.settings?.koboldCppManagedPort ?? 5001)
