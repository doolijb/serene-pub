...
describe("allocation", () => {
	it("sums only what was included", () => {
		const a = allocate(
			[
				decision(),
	...
	it("keeps excluded blocks, because that is the question people ask", () => {
		const a = allocate(
			[
				decision({
					included: false,
	...
	it("carries the score and the reason on the block itself", () => {
...
describe("rendering", () => {
	...
	it("gives world lore to the template as name-keyed JSON", () => {
	...
	it("exposes blocks under the names existing templates already use", () => {
	...
	it("excluded blocks do not reach the template", () => {
		const r = render({
	...
	it("hands the budget to the template, so a story string can react to it", () => {
		const r = render({ ...base, template: "{{budget.remaining}}" })
	...
	it("reports what the template referenced", () => {
		const r = render({
			...base,
			template:
	...
	it("prompts land as template variables", () => {
		const r = render({
			...base,
...
describe("variable extraction", () => {
	it("finds dotted paths and each-blocks, and ignores this", () => {
		expect(
			referencedVariables("{{a.b}} {{#each xs}}{{this}}{{/each}}")
		).toEqual(["a.b", "xs"])
	})

	it("an empty template references nothing", () => {
...
describe("the engine is data, not an assumption", () => {
	...
	it("a null engine means core's default", () => {
	...
	it("an extension can register its own engine and render its own templates", () => {
	...
	it("an unknown engine refuses rather than rendering as Handlebars", () => {
		...
		).toThrow(/no renderer for template engine/)
	})

	it("the refusal names what is registered, so the fix is visible", () => {
		try {
			render({ ...base, engine: "nobody.owns:this@1", template: "x" })
		} catch (e) {
	...
	it("nobody can take over an engine somebody else owns", () => {
