 ...
 * - **`async` approves immediately.** A true async proposal needs proposal
 *   storage on every consumer target; until that lands, `async` behaves as
 *   `off` with the request recorded, and the receipt says so.
 *
...
export function createReviewer(scope: {
	...
	return async (req) => {
		if (req.position === "async") {
			// v1: recorded, not parked — see the header.
			return {
				action: "approve",
				by: "system:async-review-v1",
				at: Date.now()
			}
		}
