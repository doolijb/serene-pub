...
import { getVariable, allVariables } from "@serene-pub/sdk"
...
export function sampleContext(
	...
	for (const decl of allVariables())
		for (const key of Object.keys(decl.scope))
			out[key] = renderVariable(layouts, key, sampleFor(decl, key))
	return out
}

/**
 * One key's worth of a declaration's sample.
 *
 ...
 */
function sampleFor(
	decl: { scope: Record<string, unknown>; sample: unknown },
	key: string
): unknown {
	const keys = Object.keys(decl.scope)
	if (keys.length === 1) return decl.sample
	return (decl.sample as Record<string, unknown> | undefined)?.[key]
}
