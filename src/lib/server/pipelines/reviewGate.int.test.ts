...
async function awaitReview() {
	const { pendingReviewsFor } = await import("./reviewGate")
	await vi.waitFor(
		() => {
...
async function drainReviews() {
	const { pendingReviewsFor, resolveReview } = await import("./reviewGate")
...
describe("a run parks at the gate, and a person decides", () => {
	it("approve writes exactly what was reviewed", async () => {
		call = 0
		await drainReviews()
		const { resolveReview } = await import("./reviewGate")
