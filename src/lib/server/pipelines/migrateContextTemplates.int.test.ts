...
import { migrateContextTemplates } from "./migrateContextTemplates"
import {
	SHIPPED_VARIABLE_TEMPLATES,
	renderVariable,
	seedKeyFor
} from "./variableLayouts"
import { CONTEXT_TEMPLATE_SEED_KEY } from "./contextTemplateDefaults"
...
describe("scopes are decided one at a time", () => {
	it("writes only the user when theirs is the custom one", async () => {
		...
		for (const pin of o.filter((x) => x.slot === "variables"))
			expect(["JSON", "As written"]).toContain(pin.name)
	})
