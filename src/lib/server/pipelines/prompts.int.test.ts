...
import { bootstrapPipelines, RESPOND_SPEC_ID } from "./bootstrap"
import {
	assertSelectable,
	createPrompt,
	declaredFields,
...
describe("what a node declares", () => {
	it("reports the text fields from the descriptor, not from a list in core", () => {
		...
		return declaredFields(
			...
			expect(fields).toContain("narratorName")
		})
	})
})
...
describe("what the picker is sent", () => {
	...
	it("offers only prompts from this namespace", async () => {
		const { namespaceView } = await import("./config")
