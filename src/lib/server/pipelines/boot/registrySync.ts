...
export function typeContentHash(entry: RegistryEntry): string {
	const material = {
		...
		public: entry.public,
		/**
		 * Hashed because flipping it is exactly what the freeze rule exists to
		 * stop: the ports do not move, every spec pinning the version keeps
		 * compiling, and the run quietly stops failing (or starts failing) when
		 * the node errors. That is a behaviour change wearing a compatible
		 * signature.
		 *
...
export async function syncTypeRegistry(
	...
	for (const entry of entries) {
		...
		if (row.contentHash === hash) {
			...
			// changing the hash and taking the conflict path. What can go stale
