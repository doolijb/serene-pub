...
import { NARRATE_SPEC_ID } from "./specs/narrate"
import { renderVariable } from "./variableLayouts"
...
describe("what ships", () => {
	it("seeds an immutable layout for every variable the node renders", async () => {
		...
		expect(rows[1]!.source).toBe("{{{json characters 2}}}")
	})
	...
	it("offers assembly's own layouts on assembly's step, not the context one", async () => {
		...
		expect(stepOf("Story history")).toBe("Assemble")
	...
	it("back-fills a layout a config predates", async () => {
		 ...
		 */
		const { reconcileConfigs } = await import("./configs")
		const [spec] = await db
			.select()
...
describe("the picker", () => {
	...
	it("refuses a layout written for a different variable", async () => {
		const { assertSelectable } = await import("./variableTemplates")
...
describe("the mutation gate", () => {
	...
	const gate = async (viewer: any, id: string) => {
		const { variableOptionGate } = await import("./config")
