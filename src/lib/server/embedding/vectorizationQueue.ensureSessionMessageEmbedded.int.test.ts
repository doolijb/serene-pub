/**
 * ensureSessionMessageEmbedded() is the inline, awaited counterpart to the
 ...
 * otherwise stall every queued session generation server-wide, not just this
 * one session's turn.
 *
...
beforeAll(async () => {
	dataDir = await fs.mkdtemp(
		path.join(
			os.tmpdir(),
			"serene-pub-ensure-session-message-embedded-int-test-"
		)
	)
...
async function makeSessionMessage(
	sessionId: number,
	overrides: Partial<typeof schema.sessionMessages.$inferInsert> = {}
) {
...
describe("ensureSessionMessageEmbedded (PGlite integration)", () => {
	test("embeds and persists a message that has no embedding yet", async () => {
		...
		const updated = await testDb.query.sessionMessages.findFirst({
...
describe("ensureSessionMessageEmbedded — timeout and cooldown (PGlite integration)", () => {
	beforeEach(() => {
		vi.useFakeTimers()
	})
	...
	test("times out and rejects — doesn't hang the caller — when embed() never resolves", async () => {
		...
		const resultPromise = ensureSessionMessageEmbedded(msg.id)
	...
	test("after a timeout, a second call within the cooldown window no-ops immediately, then retries once the cooldown elapses", async () => {
		...
		const session = await makeSession(user.id)
		const msg = await makeSessionMessage(session.id, {
			content: "first call hangs"
		})
		embedMock.mockImplementation(() => new Promise(() => {}))

		const firstCall = ensureSessionMessageEmbedded(msg.id)
	...
	test("a normal (non-timeout) embed() rejection does NOT trigger the cooldown", async () => {
		...
		await expect(ensureSessionMessageEmbedded(msg.id)).rejects.toThrow(
