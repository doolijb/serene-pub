 ...
 *   5. emit success
 *
 * Step 3 rejects any avatar over the media upload cap, which real SillyTavern
 * cards can exceed since the card IS a full-resolution PNG portrait. The row
 * from step 2 is already committed, so the user gets an error toast for a
 * character that exists — and because step 4 never runs, the list doesn't show
 * it until something else refreshes.
 */
...
/**
 * The square edge whose raw RGBA pixels alone clear the media upload cap, with
 * a margin so PNG's own framing can't leave the result just under it.
 *
 * Derived from the cap rather than hardcoded: this was a fixed 2048x2048 (~16MB)
 * when the cap was 10MB, and silently stopped being oversized the moment the
 * cap moved to 50MB — the import then succeeded, no warning was produced, and
 * the test failed on an absent warning rather than on the behavior it guards.
 */
const OVERSIZED_EDGE = Math.ceil(Math.sqrt(MAX_MEDIA_UPLOAD_BYTES / 4)) + 64

/**
 * A PNG whose raw bytes exceed the avatar cap, with a character card
...
function buildOversizedCharacterPng(name: string): Buffer {
	const png = new PNG({ width: OVERSIZED_EDGE, height: OVERSIZED_EDGE })
...
describe("character import partial failure", () => {
	test("an oversized avatar imports the character with a warning, not an error", async () => {
		...
		expect(png.length).toBeGreaterThan(MAX_MEDIA_UPLOAD_BYTES)
		...
		expect(
			rows.find((r) => r.name === "Oversized Hero")?.avatarMediaId
		).toBeFalsy()
