...
describe("sessionMessages:swipeLeft — generation lock (PGlite integration)", () => {
	test("waits for an in-flight withSessionTriggerLock holder on the same session before writing", async () => {
		...
		const swipePromise = sessionMessagesSwipeLeftHandler
			.handler(fakeSocket(user.id), { id: message.id } as any, noopEmit)
			.then((res) => {
		...
		const midFlight = await testDb.query.sessionMessages.findFirst({
