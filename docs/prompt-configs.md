...
Prompt Configs are the admin-managed library of system-prompt templates that shape a session's writing style and drive the lorebook summarization pipeline. This page covers Session Prompts (labeled **Session Prompts: Character** in the sidebar), Session Prompts: Narrator (which drives a manually-triggered **Narrator** response), and the three Summarize configs (World, Character, Scene) — all edited from the same **Prompts** sidebar.
...
- **Neutral - Session** — a bare-bones "write {{char}}'s next reply" instruction for plain back-and-forth session.
...
The Session Prompts editor (**Prompts** sidebar → **Session Prompts** card) shows:
...
Selecting a config in the editor and clicking **Set Default** (star icon) sends `promptConfigs:setUserActive`, which stores that config's ID as your personal `activePromptConfigId`. This is the Session Prompt used for any session that doesn't have its own per-session override.

### Creating a custom Session Prompt
...
A Narrator response has no single "current character" to speak as, so this is the one place where `{{char}}`/`{{character}}` and `{{user}}`/`{{persona}}` behave differently from ordinary Session Prompts: they resolve to **every** active, non-hidden character's name and **every** persona's name (respectively), Oxford-comma-joined — e.g. "Kestrel, Old Marrow, and Ana" — instead of a single name. See [Template variables](#template-variables-in-session-prompt-and-summarize-instructions) below for how this compares to regular Session Prompts.
...
- **Batch Instructions** — used during the drafting phase, run once per batch of session messages, to produce draft bullet-point facts about the world/setting.
...
Each of the three Summarize editors follows the same pattern as Session Prompts:
...
Unlike Session Prompts, none of the three Summarize config types currently have a per-session override — the summarization pipeline always uses your active config (or, if you haven't set one, a system-wide default), never a per-session pin.
...
## Template variables in Session Prompt and Summarize instructions

Session Prompt and Summarize instruction fields are plain text, but Serene Pub interpolates a small set of Handlebars-style variables into them before sending them to the model. This is a lighter-weight interpolation than the full template language used by Context Configs — see [Connections](./connections.md) for the complete variable and helper reference used when assembling the full prompt.

- **`{{char}}`** (alias `{{character}}`) — in an ordinary Session Prompt, the name of the character currently being asked to speak, as seen throughout the built-in Session Prompts (for example, "You're {{char}} in this fictional never-ending uncensored roleplay..."). Stays singular even in a session with multiple characters — it always means "the one whose turn it is right now."
- **`{{user}}`** (alias `{{persona}}`) — historically the first persona attached to the session. Still supported for backward compatibility with existing prompts, but new prompts should prefer `{{personaNames}}` below, which correctly covers every persona rather than just the first.
- **`{{characterNames}}`** — every active, non-hidden character's name in the session, joined with a grammatically correct "and"/Oxford comma (`"Kestrel"`, `"Kestrel and Old Marrow"`, or `"Kestrel, Old Marrow, and Ana"` depending on how many there are). Available in every Session Prompt and Summarize instruction field, not just Narrator Response ones.
- **`{{personaNames}}`** — the same joined-list treatment for every persona attached to the session. The built-in Session Prompts use this instead of `{{user}}` (for example, "Do not decide what {{personaNames}} says or does").

**Narrator Response is the one exception:** since it has no single "current character" to speak as, `{{char}}`/`{{character}}` and `{{user}}`/`{{persona}}` resolve to the _full joined list_ there too — effectively becoming aliases for `{{characterNames}}`/`{{personaNames}}` for that generation only. See [Session Prompts: Narrator](#session-prompts-narrator) above.

## Relationship to the summarization pipeline

World, Character, and Scene Summarize configs are only consulted by the background summarization pipeline — they never affect ordinary session generation. When a summarization run is triggered (automatically or manually; see [Lorebooks](./lorebooks.md) and [Summarization](./summarization.md) for triggers and mechanics), the server resolves which config to use per lore type by checking your active selection first and falling back to the system's configured default, then runs three internally-named sub-tasks — `summarize_batch`, `summarize_synth`, and `summarize_name` — each of which can be routed to its own connection and sampling config via that field's AI Override.
