...
- **Browser console shows "Mixed Content... has been blocked."** The socket endpoint resolved to `http://` on an `https://` page — set `PUBLIC_URL=https://<your hostname>`, and check your proxy sets `X-Forwarded-Proto` (`TRUSTED_PROXIES` is what makes that header believed).
...
- **Socket requests 404 at `/socket.io/...`, or a "Socket connection timeout" with no CORS/404 error at all.** Real-time updates share the app's own port, so there is no second server to route to. Either your proxy is intercepting `/socket.io/` and sending it somewhere else (a leftover rule pointing at the old `:3001`), or it isn't forwarding the WebSocket upgrade (`Upgrade`/`Connection` headers).
