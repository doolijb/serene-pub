...
- **Browser console shows "Mixed Content... has been blocked."** Not the real-time connection — that is opened against the page's own origin, so its scheme always matches. Something else is emitting an `http://` URL on an `https://` page, which happens when Serene Pub cannot tell it is being served over HTTPS: set `PUBLIC_URL=https://<your hostname>`, and check your proxy sets `X-Forwarded-Proto` (`TRUSTED_PROXIES` is what makes that header believed).
- **"blocked by CORS policy" pointing at your own domain.** The `Origin` header the browser sent doesn't match the `Host` header your proxy forwarded — usually a proxy rewriting `Host` to an internal name. Forward the real one (`proxy_set_header Host $host`), or set `PUBLIC_URL` to the hostname your users actually type, which allowlists it.
- **Socket requests 404 at `/socket.io/...`, or a "Socket connection timeout" with no CORS/404 error at all.** Real-time updates share the app's own port, so there is no second server to route to. Either your proxy is intercepting `/socket.io/` and sending it somewhere else (a leftover rule pointing at the old `:3001` — see [Upgrading from 0.5.2 or earlier](./hosting.md#upgrading-from-052-or-earlier)), or it isn't forwarding the WebSocket upgrade (`Upgrade`/`Connection` headers).
