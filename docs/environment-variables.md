...
| `SERENE_PUB_NO_PAUSE`              | unset                                     | Not read by the app itself — read by the bundled `run.sh` / `run.cmd` launcher scripts. Set to `1` or `true` to stop them from pausing for a keypress after a failed start, which they otherwise do so a double-clicked window doesn't close before you can read the error. |
